...
#записываем сроки действия всех сертов в файле в массив, затем каждый должен будет обрабатываться функцией timestamp_calc
 ...
 "
			...
			#без записи в файл -- пробелы при выводе, либо отсутсвие названия файла	(пофикшено)
			#echo $file | tr -d ' ' | tr '\r' '\n' > "${DIR}/${secret}/${secret}.txt" 

			# посмотреть вручную файлы секрета
			#1. вариант -- один вывод. oc get secret istio-egress-api-gateway-certs -o json | jq  '.data."tls.crt"' | base64 -di
			#2. вариант -- второй вывод. oc get secret istio-egress-api-gateway-ca-certs -o json | jq  '.data."ca.crt"' | base64 -di
